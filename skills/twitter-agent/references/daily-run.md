...
If `pending_recovery` is non-empty, present the complete set and stop for a decision as defined by the Pending Recovery section in [SKILL.md](../SKILL.md). Undecided recovery blocks plan creation; a successful durable scheduler skip does not.
...
tw scheduler skip <scheduled_task_ref> --json
```
