...
Scope the guide to the affected component. A missing, incompatible, or approved skill update uses `skills_only` and never runs a CLI installer, `tw update`, `tw setup`, daemon repair, or native-host registration. A CLI-only update uses `cli_only` and never fetches or installs skills. When both are pending, use `skills_and_cli`. These labels describe component scope, not an installation sequence. Reserve `full_setup` for initial setup or missing/unknown core runtime readiness.

When the user approves the update, use the guide as a download reference for the selected components and current host. Choose available reading tools and an appropriate installation approach; the HTML version at `https://www.threadwave.xyz/cli/setup` is equivalent to the Markdown guide. Handle retrieval and installation yourself where tools permit, asking for help only when needed. After the update, run one preflight selected by the contract and resume the preserved request. Use regular preflight for legacy mode or a changed CLI version, install mode, adapter, or launcher; use forced preflight only for the same confirmed receipt-aware CLI. User permissions, sign-in, and payment remain user-owned. Setup or update success is not approval for strategy, content, posting, replying, or another X mutation.
